...
class SimpleTokenizer(object):
    def __init__(self, bpe_path = default_bpe(), token_length = 16384):
        self.byte_encoder = bytes_to_unicode()
        self.byte_decoder = {v: k for k, v in self.byte_encoder.items()}
        merges = gzip.open(bpe_path).read().decode("utf-8").split('\n')
        merges = merges[1:token_length - 256 - 2 + 1]
        ...
        self.vocab_size = token_length + 256 # 16640
    ...
    def decode(self, tokens, remove_start_end = True, pad_tokens = set()):
        ...
        text = ''.join([self.decoder[token] if token in self.decoder.keys() else '<mask>' for token in tokens ])
